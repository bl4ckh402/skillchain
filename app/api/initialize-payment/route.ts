...
export async function POST(request:any) {
  try {
    ...
    if (data.paymentType === 'job_posting') {
      // Job posting payment
      const amountInCents = Math.round(10 * 100); // $10 USD in cents
      const metadata = {
      ...
      const response = await axios({
        ...
        data: {
          ...
          callback_url: `${process.env.NEXT_PUBLIC_APP_URL}/jobs/post/payment/verify`,
