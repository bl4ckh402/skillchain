  ...
  return (
    <div className="fixed top-4 left-4 w-[240px] h-[135px] shadow-lg rounded-xl overflow-hidden z-10">
      <div className="w-full h-full" style={{ aspectRatio: "16/9" }}>
        <ParticipantView participant={participant} muteAudio={false} />
      </div>
    </div>
  );
};
...
export default function CallLayout() {
  const call = useCall();
  const { useLocalParticipant, useParticipants, useCallCallingState } =
    useCallStateHooks();
    ...
    <div className="flex flex-col h-screen bg-slate-950">
      ...
      <ResizablePanelGroup direction="horizontal" className="flex-1">
        ...
        <ResizablePanel defaultSize={25} minSize={20}>
          <Card className="h-full border-l rounded-none border-slate-800">
              ...
              <div className="p-4 border-t border-slate-800">
                <form className="flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 px-3 py-2 text-white border rounded-md bg-slate-800 border-slate-700 placeholder:text-slate-400"
                  />
                  <Button
                    type="submit"
                    variant="default"
